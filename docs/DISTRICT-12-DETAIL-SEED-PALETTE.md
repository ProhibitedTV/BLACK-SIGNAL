...
The Cyberpunk Streets audit on the current GameGuru MAX install found 268 FPE assets. CITY V3 and DETAIL V1 can only clone entities that are already loaded by the level, so optional DLC props still need one hidden exemplar in `BLACK SIGNAL - District 12.fpm` before runtime code can propagate them.
...
- DETAIL V1: benches, bus stops, ATMs, dumpsters, fireplugs, curb furniture, service clutter, boxes, cardboard, newspaper piles.
