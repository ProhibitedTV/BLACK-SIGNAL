...
if (-not (Test-Path $PlanPath)) {
    throw ('Block 01 build plan not found: {0}' -f $PlanPath)
}
if (-not (Test-Path $ListPath)) {
    throw ('District 12 dependency list not found: {0}' -f $ListPath)
}
...
if ($rows.Count -eq 0) {
    throw ('Block 01 build plan is empty: {0}' -f $PlanPath)
}

Write-Host 'BLACK SIGNAL - District 12 Block 01 build checklist'
Write-Host ('Plan: {0}' -f $PlanPath)
Write-Host ('Level dependency list: {0}' -f $ListPath)
Write-Host ''
...
foreach ($row in $rows) {
    if ($row.phase -ne $currentPhase) {
        ...
        $currentZone = $null
        Write-Host ''
        Write-Host ('=== PHASE {0} ===' -f $currentPhase)
    }

    if ($row.zone -ne $currentZone) {
        $currentZone = $row.zone
        Write-Host ''
        Write-Host ('[{0}]' -f $currentZone)
    }
...
Write-Host ''
Write-Host 'Summary'
Write-Host ('  Ready in current District 12 list: {0}' -f $ready)
Write-Host ('  Missing required plan assets: {0}' -f $requiredMissing)
Write-Host ('  Missing optional detail assets: {0}' -f $optionalMissing)
Write-Host ''
